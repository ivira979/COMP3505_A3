...
void setUp() throws Exception {
	...
	keyedValues = mock(KeyedValues.class);
	  ...
	  when(keyedValues.getIndex(2)).thenReturn(2);
		  ...
		  ArrayList keyList = new ArrayList<>(); keyList.add(0); keyList.add(1);
		  keyList.add(2); when(keyedValues.getKeys()).thenReturn(keyList);
		  ...
		  ArrayList keyList2 = new ArrayList<>(); keyList2.add(0); keyList2.add(1);
		  keyList2.add(2); keyList2.add(3);
...
/*getCumulativePercentages tests
 1. values at 0(5/16), 1((5+9)/16), 2((5+9+2)/16) -> 0.3125, .875, 1.0 
 2. keys at 0, 1, 2 -> 0, 1, 2 
 3. indexes at 0, 1, 2 -> 0, 1, 2
 4. values at 0 (5/15), 1(5+0/15), 2 (5+0+8)/15, 3 (5+0+8+2)/15 -> .33, .33, .866.., 1.0
 5. values at 0 (0/16), 1(0+5/16), 2(0+5+9/16), 3(0+5+9+2/16) -> 0.0, 0.3125, 0.875, 1.0
 6. values at 0 (0/0) -> NaN
 7. 0 (5/5), 1 (5+0/5) -> 1.0, 1.0
 8. -5/0, -5+5/0 -> NaN*/
 
 

  @Test 
  void testCumulativePercentages_1() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues);
  
  assertAll( () -> assertEquals(0.3125, actual.getValue(0).doubleValue(),
  0.1d), () -> assertEquals(0.875, actual.getValue(1).doubleValue(), 0.1d), ()
  -> assertEquals(1, actual.getValue(2).doubleValue(), 0.1d) );
 
  
  }
  
  @Test void testCumulativePercentages_2() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues);
  
  assertAll( () -> assertEquals(0, actual.getKey(0)), () -> assertEquals(1,
  actual.getKey(1)), () -> assertEquals(2, actual.getKey(2)) );
 }
  
  @Test void testCumulativePercentages_3() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues);
  
  assertAll( () -> assertEquals(0, actual.getIndex(0)), () -> assertEquals(1,
  actual.getIndex(1)), () -> assertEquals(2, actual.getIndex(2)) );
  }
  
  @Test void testCumulativePercentages_4() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues2);
  ...
  @Test void testCumulativePercentages_5() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues3);
  ...
  @Test void testCumulativePercentages_6() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues4); assertEquals(0.0/0.0,
  actual.getValue(0).doubleValue());  }
  
  @Test void testCumulativePercentages_7() { KeyedValues actual =
  DataUtilities.getCumulativePercentages(keyedValues5); assertAll( () ->
  assertEquals(1, actual.getValue(0).doubleValue(), 0.1d), () ->
  assertEquals(1, actual.getValue(1).doubleValue(), 0.1d) );
 }
