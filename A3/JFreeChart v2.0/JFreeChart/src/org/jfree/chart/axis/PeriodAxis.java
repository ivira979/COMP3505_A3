...
public class PeriodAxis extends ValueAxis 
    ...
    /**
     * Returns the size of the axis given a fixed width but no height 
     * constraint.  We override the base class method to handle the multiple
     * bands that can be displayed by this axis.
     * 
     * @param g2  the graphics device.
	...
	protected ArrangeResult arrangeFN(Graphics2D g2, double fixedWidth, 
        ...
        if (h < 0.0) {
            ...
            h = h + Math.max(this.minorTickMarkOutsideLength, 
    ...
    protected AxisState drawTickLabels(int band, Graphics2D g2, AxisState state,
        ...
        long ww = Math.round(
            java2DToValue(area.getX() + w + 5.0, area, location)
        ) - axisMin;
        ...
        while (p.getFirstMillisecond(this.timeZone) <= axisMax) {
            float x = (float) valueToJava2D(
                p.getMiddleMillisecond(this.timeZone), area, location
            );
            ...
            if (last > axisMax) {
                ...
                if ((x + bb.getWidth() / 2) > area.getMaxX()) {
                    float xstart = (float) valueToJava2D(
                        Math.max(first, axisMin), area, location
                    );
            ...
            if (first < axisMin) {
                ...
                if ((x - bb.getWidth() / 2) < area.getX()) {
                    float xlast = (float) valueToJava2D(
                        Math.min(last, axisMax), area, location
                    );
